...
#load(url("https://github.com/rl627/SALURBAL-Covid19-Internal-data/blob/master/covid19_processed_data_dev.rdata?raw=true"))
...
server <- function(input, output) {
  ...
  #### Tab 2: Daily ####
  output$plot_rolling  = renderPlot({
  ...
  #### Tab 4: Data ####
  tidy.full = full_global %>% 
    select(level, country, loc,salid1, salid2, date, confirmed, deaths) %>% 
    pivot_longer(cols = c(confirmed,deaths),names_to = "count_type",values_to = "count") %>% 
    arrange(level, country,loc, count_type,date) %>% 
    filter(!is.na(count))
  ...
  data_output_clean_react = reactive({
    ...
    if (input$data_level4=="country") {data_tmp %>% select(-salid1, -salid2, -country) %>% 
        ...
        select(Outcome = count_type,Level = level, Country = country, "L1" = salid1, "L1 Name"= loc,date,count )}
    else if (input$data_level4=="L2") {data_tmp %>% select(-salid1)%>% 
        select(Outcome = count_type,Level = level, Country = country, "L1" = salid2, "L1 Name"= loc,date,count )}
    })
