...
p = df_tmp %>% 
  ...
  scale_x_continuous(breaks=seq(0, as.numeric(max(pull(df_tmp,days.since.100))), by=10), limits=c(0, as.numeric(max(pull(df_tmp,days.since.100)))+5))+
...
plot_cumulative_count_rate_onset = function(df_tmp){
  ...
  p=df_tmp %>% 
    ...
    scale_size_manual(values = df_tmp %>% select(loc, size) %>% distinct() %>% pull(size) )+
    scale_linetype_manual(values=c(2, 1))+
    scale_x_continuous(breaks=seq(0, as.numeric(max(pull(df_tmp,days.since.100)))+5, by=10), limits=c(0, as.numeric(max(pull(df_tmp,days.since.100)))+5))+
    scale_y_log10(limits=c(y_start_tmp, 10^nchar(trunc(max(pull(df_tmp,confirmed))))))+
    annotation_logticks(sides="l") +
    labs(title=title_tmp,
         subtitle = subtitle_tmp,
...
plot_cumulative_death_onset = function(df_tmp){
  ...
  reference_tmp$y5<-y_start_tmp*(2^(1/7))^(reference_tmp$days.since.10)
    ...
    scale_color_manual(values= df_tmp %>% select(loc, colors) %>% distinct() %>% pull(colors))+
    scale_size_manual(values = df_tmp %>% select(loc, size) %>% distinct() %>% pull(size) )+
    scale_x_continuous(breaks=seq(0, as.numeric(max(pull(df_tmp,days.since.10))), by=10), limits=c(0, as.numeric(max(pull(df_tmp,days.since.10)))+5))+
...
plot_cumulative_death_rate_onset = function(df_tmp){
  ...
  p=df_tmp %>% 
    ...
    scale_size_manual(values = df_tmp %>% select(loc, size) %>% distinct() %>% pull(size) )+
    scale_linetype_manual(values=c(2, 1))+
    scale_x_continuous(breaks=seq(0, as.numeric(max(pull(df_tmp,days.since.10)))+5, by=10), limits=c(0, as.numeric(max(pull(df_tmp,days.since.10)))+5))+
    scale_y_log10(limits=c(y_start_tmp, 10^nchar(trunc(max(pull(df_tmp,deaths))))))+
    annotation_logticks(sides="l") +
    labs(title=title_tmp,
         subtitle = subtitle_tmp,
