...
## Latest Date
date_str = paste0("Last update: ",max(full$date)," 11:59PM" )
...
full_wide = full %>% 
    ...
    summarise(v_confirmed = paste(confirmed, collapse = ","),
              v_deaths = paste(deaths, collapse = ",")) %>% 
    ungroup()
...
dt1_global =  full %>% filter(date == max(full$date)) %>% 
    ...
    left_join(full_wide) %>% 
    select(Country = loc, 
           `case_trend`= v_confirmed, 
           Confirmed = confirmed,
           `death_trend`= v_deaths,
           Deaths = deaths,
...
colDefs = list(list(targets =c(1),render = JS(js) ))
...
server <- function(input, output) {
    ...
    dt2_global = reactive({
        if (input$data_type == "Cases"){dt1_global %>% select(Country,case_trend,Confirmed) }
        else {dt1_global %>% select(Country,death_trend,Deaths) %>% arrange(desc(Deaths))}
    })
    
    
    output$dt1 = DT::renderDataTable({
        dt_out = datatable(dt2_global(), 
                           rownames = FALSE,
                           options = list(columnDefs = colDefs, 
                                          fnDrawCallback = cb_bar,
                                          sDom  = '<"top">rt<"bottom">',
                                          pageLength = 10))
        dt_out$dependencies = append(dt_out$dependencies, htmlwidgets:::getDependency("sparkline"))
        dt_out
    })
