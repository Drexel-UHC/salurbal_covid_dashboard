...
ui = function(){
  ui <- fluidPage(theme = shinytheme("flatly"),
                                 ...
                                 tabPanel("Cumulative",
                                          ...
                                          sidebarLayout(
                                            sidebarPanel(width  = 3,
                                                         ...
                                                         fluidRow(column(width = 12,strong("Plot Options"),align = 'center')),
                                 ...
                                 tabPanel("Daily",
                                          sidebarLayout(
                                            sidebarPanel(width  = 3,
                                                         ...
                                                         sliderInput("smooth_2","Number of Days to smooth:",
                 ...
                 research examining the connections between the environment and human health."),
                                          ...
                                          h4("Version updates:"),
                                          tags$ul(
                                            tags$li("5/6/2020: Subnational data for Chile and Colombia. Added L1 visualizations to map.  ")
                                          ),
...
server <- function(input, output) {
  ...
  # }
  # waiter_hide()
  # w <- Waiter$new(id = c("dt1","plot_count", "plot_rolling","mapper","data_dt","data_available"),
  #                 html = spin_refresh(),
  #                 color = "$ffffff00")
  ...
  output$control_state_city_1 = renderUI({
    ...
    if (input$level_1 == "state"){
      ...
      label_tmp = paste("Select", input$level_1)
  ...
  dt2_global = reactive({
    ...
    type_name_tmp = case_when(
      input$outcome_1== "cases"~"Confirmed",
      input$outcome_1!= "cases"~"Deaths")    
    df_tmp = `dt1_global` %>% 
      filter(level == input$level_1) %>% 
  ...
  DT_selected_countries = reactive({
    if(length(input$dt1_rows_selected)==0){NA_character_}
      ...
      else if (input$level_1 =="L1"){dt2_global() %>% slice(input$dt1_rows_selected) %>% pull(L1)}
      else if (input$level_1 =="L2"){dt2_global() %>% slice(input$dt1_rows_selected) %>% pull(L2)}
    }
  })
  ...
  output$control_state_city_2 = renderUI({
    if (input$level_2 == "country"){}
      ...
      top1_tmp = choices_df %>% filter(level == "state",rate =="count",country == input$countries_2) %>% filter(top1 == "best") %>% pull(state)
      states_top_tmp  = choices_df %>% filter(level == "state",rate =="count",country == input$countries_2) %>% filter(top == "top") %>% pull(state)
      states_rest_tmp  = choices_df %>% filter(level == "state",rate =="count",country == input$countries_2) %>% filter(top == "rest") %>% pull(state)
      ...
      top1_tmp = choices_df %>% filter(level == input$level_2,rate =="count",country == input$countries_2,top == "top") %>% filter(top1 == "best") %>% pull(state)
      states_top_tmp  = choices_df %>% filter(level == input$level_2,rate =="count",country == input$countries_2,top == "top") %>% pull(state)
      states_rest_tmp  = choices_df %>% filter(level == input$level_2,rate =="count",country == input$countries_2) %>% filter(top == "rest") %>% pull(state)
      states_non_tmp = choices_df %>% filter(level == input$level_2,rate =="count",country == input$countries_2) %>% filter(top == "non") %>% pull(state)
      pickerInput("state_city_2",label = paste("Select",input$level_2),
                  multiple = FALSE,
                  choices = list(
                    "Top 10 Largest" = states_top_tmp,
                    "Others" =  states_rest_tmp,
  ...
  tidy.daily.subnational.react = reactive({
    ...
    req(!is.na(input$smooth_2))
    
    if (input$level_2 == "country"){
      tidy.daily.country %>% filter(smooth_days == input$smooth_2)
    }
    else { 
      ...
      tidy.daily.subnational %>% 
        ...
        filter(smooth_days == input$smooth_2)}
  ...
  output$plot_rolling  = renderCachedPlot({
    ...
    if (input$level_2 =="country"){
      plot_lockdown_effect(tidy.daily.subnational.react(),input$countries_2) }
    else {
      plot_lockdown_effect_salurbal(tidy.daily.subnational.react()) }
    },
    cacheKeyExpr = { list(input$smooth_2,input$countries_2,tidy.daily.subnational.react()) })
