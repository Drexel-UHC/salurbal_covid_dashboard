...
tidy.data.all = bind_rows(tidy.data.all.old,tidy.data.all.new) %>% 
  ...
  mutate(cum_value = ifelse( str_detect(type_rate,"positivity"),
                            9999,
                            cum_value)) %>% 
  filter(!type_rate%in%c('tests count','tests rate'))
...
tidy.cumulative = tidy.data.all  %>% 
  filter(type_rate!="positivity count") %>% 
  select(-daily_value) %>% 
  rename(n = cum_value)%>% 
  left_join(xwalk_data_rate) %>% 
  left_join(xwalk_data_rate_cleaned) %>% 
...
server <- function(input, output) {
  ...
  output$control_outcome_1 = renderUI({
    ...
    if(input$level_1 == "country"){
      ...
      else if (length(available_tmp)==1){
        radioGroupButtons("outcome_1",
                          #"Plot Outcome",
                          choices = c("Cases"="cases"),
                          justified = T,
      ...
      else if (length(available_tmp)==3){
        radioGroupButtons("outcome_1",
                          #"Plot Outcome",
                          choices = c("Cases"="cases",
                                      "Deaths"="deaths",
