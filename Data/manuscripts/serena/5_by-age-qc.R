...
{## 1. Check cumulative numbers
  ...
  qc = df_merge%>% 
    ...
    geom_point()+
    facet_grid(vars(level), vars(name), scales = 'fixed')+
...
{## 1. Check Trends numbers
  ...
  df_merge2
  df_merge2 %>% 
    pivot_longer(values_from=c(value_old, value_new))
  ...
  filter(name%in%c("cases","deaths")) %>% 
    ...
    qc2 = df_merge2 %>% 
    filter(level == "L1") %>% 
    ggplot(aes(x=date, y = value_, value_new, label = salid))+
    geom_point()+
